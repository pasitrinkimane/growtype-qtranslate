...
function qtranxf_language_column_header($columns)
{
    ...
    $columns = apply_filters('growtype_qtranslate_manage_columns', $columns);
...
{
    return !in_array($post_type, apply_filters('qtranxf_post_type_optional', ['revision', 'nav_menu_item']));
}
...
function qtranxf_config_add_form(&$page_config, $name)
{
    ...
    if (!isset($page_config['forms'][$name])) {
        ...
        if (!isset($page_config['forms'][$name]['fields'])) {
