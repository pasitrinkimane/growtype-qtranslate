...
function qtranxf_add_language_menu_item(&$items, &$menu_order, &$itemid, $key, $language)
{
    global $q_config;
    $item = $items[$key];
    $flag_location = trailingslashit(content_url()) . qtranxf_flag_location_default();
    $altlang = null;
    $url = '';
    //options
    $type = 'LM'; // [LM|AL]
    $title = ''; // [none|Language|Current]
    $current = false; // [shown|hidden]
    $flags = true; // [none|all|items]
    $lang_names = false; // names=[shown|hidden]
...
function qtranxf_pre_get_posts($query)
{//WP_Query
...
function qtranxf_excludeUntranslatedPosts($where, $query)
{//WP_Query
    switch ($query->query_vars['post_type']) {
        //known not to filter
        case 'nav_menu_item':
 ...
 */
function qtranxf_cache_delete_metadata($meta_type, $object_id)
{//, $meta_key) {
    global $q_config;
 ...
 * Force main language
 */
function qtranxf_force_main_language()
{
    global $q_config;
